...
public:
	...
	************************************************/
	void chooseDiscard() {
		...
		cout << "please choose a card to discard" << endl;

		//prompts the player to input the number and color of the card to be discarded and stores the values 
...
int main()
{
	...
	//set up the game by calling createCards, shuffle and deal
	createCards(deck);
	shuffle(deck);
	deal(players, deck);

	//while there are crads still in the deck
	while (deck.size() != 0) {

		cout << "New Round begins" << endl << endl;

		//for each player draw a new card, then have them choose a card to discard
		for (int i = 0; i < 3; i++) {

			cout << "player " << i + 1 << "'s turn" << endl;
			players[i].draw(deck);
			players[i].chooseDiscard();
